...
import 'package:sudoku_trainer/screens/result_screen.dart';
...
class SudokuBoardScreen extends StatelessWidget {
  ...
  final String? Function() strategyExplanation; // ← new field
  final bool generalMode;

  const SudokuBoardScreen({
    Key? key,
    required this.difficulty,
    //required bool Function(int p1, int p2, int p3) activeStrategy,
    required this.strategyExplanation, // ← required here
    this.generalMode = false, // Default to false (specific strategy mode)
  }) : super(key: key);
  ...
  void checkGameCompletion(BuildContext context) {
    ...
    // Check if the board is complete (no empty cells) and valid
    if (provider.isBoardComplete() && provider.isBoardValid()) {
      // Navigate to result screen with success
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (context) => ResultScreen(isSuccess: true)),
      );
    }
  }
              ...
              SafeArea(
                child: SingleChildScrollView(
                  padding: const EdgeInsets.only(
                    bottom: 160,
                  ), // extra room for AI box
                  child: Column(
                    key: ValueKey(
                      '${provider.sudokuBoard}_${provider.mistakeExplanation}',
                    ),
...
class FloatingPatternHintButton extends StatefulWidget {
  ...
  @override
  State<FloatingPatternHintButton> createState() =>
      _FloatingPatternHintButtonState();
}
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [
                  BoxShadow(
                    color: (isClear ? Colors.red : Colors.deepPurple)
                        .withOpacity(0.4),
                    blurRadius: 12,
...
class _AIMessageBoxState extends State<AIMessageBox>
  ...
  void initState() {
    ...
    _slideAnimation = Tween<Offset>(
      ...
      CurvedAnimation(parent: _fadeSlideController, curve: Curves.easeOutCubic),
    );
        ...
        child: FadeTransition(
          ...
          child: AnimatedBuilder(
            ...
            builder: (context, child) {
              final offset =
                  isDanger
                      ? Offset(_shakeController.value * 4, 0)
                      : Offset.zero;
